...
if [[ "$all_keypair_names" == *"$karpenter_key_name"* ]]; then
    ...
    create_key_object=$(aws ec2 create-key-pair \
        --key-name "${karpenter_key_name}" | jq -r ".KeyMaterial" > ~/.ssh/${karpenter_key_name}.pem)
    echo ${create_key_object}
fi

if [[ $(aws cloudformation describe-stacks --stack-name ${stack_name} --query 'Stacks[].StackName' --output text) != ${stack_name} ]]; then
    vpc_stack_object=$(aws cloudformation create-stack --stack-name ${stack_name} \
        --capabilities CAPABILITY_IAM \
        --template-body file://./${vpc_cf_stack_file} \
        --region ${region_name})
    echo ${vpc_stack_object}
fi
